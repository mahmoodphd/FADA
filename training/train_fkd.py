#!/usr/bin/env python3
"""Train Fetal VLM from scratch with FULL KD on the mixed dataset.
...
Unlike exp2_mixed, this experiment uses the FULL teacher feature cache that
covers ALL images (original + interpretation), so KD loss is applied to
every batch, not just original dataset batches.

Usage:
    python train_exp2_mixed_full_kd.py
    python train_exp2_mixed_full_kd.py --output-dir outputs_mixed/exp2_mixed_full_kd
    python train_exp2_mixed_full_kd.py --resume
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Fetal VLM from scratch with full KD on mixed data (exp2_mixed_full_kd)"
    )
    ...
    parser.add_argument(
        "--cache-path",
        default=str(PROJECT_ROOT / "processed_data" / "teacher_feature_cache_full.h5"),
    )
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "outputs_mixed" / "exp2_mixed_full_kd"))
    ...
    # ---- Environment ----
    logger.info("=" * 60)
    logger.info("  Fetal VLM exp2_mixed_full_kd (from scratch, KD fusion_mse, full KD coverage)")
    logger.info("=" * 60)
    ...
    # ---- Train ----
    logger.info("Starting exp2_mixed_full_kd KD training (fusion_mse, w_feat=%.2f)...", args.w_feat)
    ...
    logger.info("=" * 60)
    logger.info("  exp2_mixed_full_kd KD Training Complete!")
    logger.info("=" * 60)
    ...
    metadata = {
        ...
        "data": "mixed_train.jsonl with full KD coverage (all images, original + interpretation)",
    }
