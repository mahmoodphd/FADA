...
Parses JSON responses from the Qwen3VL model into structured detection,
segmentation, and classification predictions for metric computation.
...
# Qwen VL coordinate normalization range (must match formatter.py)
COORD_SCALE = 1000
...
class DetectionPrediction:
    ...
    label: str
    bbox_xyxy: Tuple[int, int, int, int]  # [x_min, y_min, x_max, y_max] in [0, 1000)
    confidence: float = 1.0


@dataclass
class SegmentationPrediction:
    """A single segmentation prediction."""
    label: str
    bbox_xyxy: Tuple[int, int, int, int]
    mask_polygon: List[Tuple[int, int]]  # [(x, y), ...]
    confidence: float = 1.0
...
class KeypointPrediction:
    ...
    label: str
    bbox_xyxy: Tuple[int, int, int, int]
    keypoints: List[Tuple[int, int, int]]  # [(x, y, visibility), ...]
    confidence: float = 1.0


@dataclass
class ParsedOutput:
    """Container for all parsed predictions from a single model response."""
...
def parse_model_output(text: str) -> ParsedOutput:
    ...
    The model is expected to output JSON (either a single object or an array).
    This parser handles:
    - Clean JSON arrays: [{"bbox_2d": [...], "label": "..."}, ...]
    - Single JSON objects: {"label": "..."}
    - JSON embedded in surrounding text
    - Common formatting issues (trailing commas, missing brackets)

    Args:
        text: Raw text output from the model.

    Returns:
        ParsedOutput with extracted predictions.
    """
...
def _extract_json(text: str) -> Optional[Any]:
    ...
    2. Find JSON array or object patterns
    3. Try to fix common formatting issues
    """
    text = text.strip()
    ...
    # Strategy 2: Find JSON array [...] or object {...}
    # Try array first (more common for multi-detection)
...
def _parse_item(item: Dict[str, Any], result: ParsedOutput) -> None:
    ...
    # Accept both "bbox_2d" (new native format) and "box_2d" (legacy fallback)
    ...
    # Classification (no bbox)
    if bbox_2d is None:
        result.classifications.append(
            ClassificationPrediction(label=label)
        )
        return

    # Validate bbox_2d: [x_min, y_min, x_max, y_max]
    ...
    # Segmentation prediction (has mask)
    if mask is not None:
        try:
            ...
            if len(polygon) >= 3:
                ...
                result.parse_errors.append(f"Mask polygon too few points: {len(polygon)}")
        except (ValueError, TypeError, IndexError) as e:
            result.parse_errors.append(f"Invalid mask polygon: {e}")
        return

    # Detection prediction (box only)
    result.detections.append(
        DetectionPrediction(label=label, bbox_xyxy=bbox)
    )
...
def rescale_predictions_to_original(
    ...
    Args:
        parsed: ParsedOutput with coordinates in [0, 1000) normalized space.
        original_wh: (width, height) of the original image.

    Returns:
        New ParsedOutput with coordinates in original pixel space (xyxy order).
    """
    ow, oh = original_wh
    ...
    result = ParsedOutput(raw_text=parsed.raw_text, parse_errors=list(parsed.parse_errors))
    ...
    for s in parsed.segmentations:
        ...
        result.segmentations.append(
            SegmentationPrediction(
                label=s.label,
                bbox_xyxy=scale_box(s.bbox_xyxy),
    ...
    for k in parsed.keypoints:
        ...
        result.keypoints.append(
            KeypointPrediction(
                label=k.label,
                bbox_xyxy=scale_box(k.bbox_xyxy),
